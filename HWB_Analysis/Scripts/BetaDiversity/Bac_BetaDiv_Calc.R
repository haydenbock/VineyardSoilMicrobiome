...
otu_table_data_2019 <- as.data.frame(otu_table(ps.soil.b.19))
otu_table_data_2020 <- as.data.frame(otu_table(ps.soil.b.20))
...
beta_diversity_metrics_2019 <- beta_diversity_metrics_2019[1:35, ] #35 corresponds to sample num in 2019 OTU table 


beta_diversity_metrics_2020 <- as.data.frame(beta_diversity_metrics_2020)
beta_diversity_metrics_2020 <- beta_diversity_metrics_2020[1:47, ] #this is to remove the repeating rows that occur for some reason in the calcs. 47 corresponds to sample num in 2020 OTU table
...
#Genera LEVEL====
#load phyloseq objects
ps.b.19.gen <- ps.16s.2019.gen #bacteria by genera 2019
ps.b.20.gen <- ps.16s.2020.gen #bacteria by genera 2020


#Subset soil only compartment
ps.soil.b.19 <- subset_samples(ps.b.19.gen, type == "soil")
ps.soil.b.20 <- subset_samples(ps.b.20.gen, type == "soil")

# Step 1: Extract the OTU table from the phyloseq object
otu_table_data_2019 <- as.data.frame(otu_table(ps.soil.b.19))
otu_table_data_2020 <- as.data.frame(otu_table(ps.soil.b.20))

otu_table_data_binary.19 <- otu_table_data_2019 > 0
otu_table_data_binary.20 <- otu_table_data_2020 > 0
...
#Phylum LEVEL ----
#load phyloseq objects
ps.b.19.phy <- ps.16s.2019.phy #bacteria by phylum 2019
ps.b.20.phy <- ps.16s.2020.phy #bacteria by phylum 2020
